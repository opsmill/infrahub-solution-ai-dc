...
- **Render per-vendor configuration from one model** — the same design produces Cisco, Arista, Dell, Juniper Junos, SONiC, and Cumulus Linux startup configs, each device routed to its vendor's template automatically
...
- **Transforms and artifacts** — per-vendor startup configuration (Cisco/Arista/Dell/Juniper/SONiC/Cumulus Jinja2 templates), cabling plan (Python), and computed interface descriptions
- **Demo data** — six single-vendor fabrics (Fabric-A: Cisco; Fabric-B: Arista; Fabric-C: Dell; Fabric-D: Juniper; Fabric-E: SONiC; Fabric-F: Cumulus Linux), seed overlay tenants (`Blue` on Fabric-A, `Green` on Fabric-D, `Purple` on Fabric-E, `Amber` on Fabric-F), plus device types, IPAM and overlay pools, interface profiles, and device templates
