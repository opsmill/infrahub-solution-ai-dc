...
- **Render per-vendor configuration from one model** — the same design produces Cisco, Arista, Dell, Juniper Junos, and SONiC startup configs, each device routed to its vendor's template automatically
...
- **Transforms and artifacts** — per-vendor startup configuration (Cisco/Arista/Dell/Juniper/SONiC Jinja2 templates), cabling plan (Python), and computed interface descriptions
- **Demo data** — five single-vendor fabrics (Fabric-A: Cisco; Fabric-B: Arista; Fabric-C: Dell; Fabric-D: Juniper; Fabric-E: SONiC), seed overlay tenants (`Blue` on Fabric-A, `Green` on Fabric-D, `Purple` on Fabric-E), plus device types, IPAM and overlay pools, interface profiles, and device templates
