...
- The route-reflector-client relationship is derived from tier ordering in the template (super-spine → spine
  → leaf), so no `route_reflector` attribute is stored on devices.
