...
#
#   ./dev/compare_versions.sh 1.10.6 1.11.0
#
...
# releases is invisible. Those changes are real: 1.11.0b1 raised the nofile ceiling to 1048576 on the
# database, server and worker, added a Neo4j bolt thread-pool knob, and added a merge grace period,
# all relative to 1.10.6. (1.11.0b1 and 1.11.0b2 ship an identical stack definition -- verified by
# checksum -- so between those two only the application image differs.)
#
