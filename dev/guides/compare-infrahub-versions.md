...
```bash
inv compare-versions --baseline=1.10.6 --candidate=1.11.0   # or: ./dev/compare_versions.sh 1.10.6 1.11.0
```
...
```bash
inv test-version --version=1.11.0              # -> perf-results/1.11.0.json
```
...
uv run python dev/compare_runs.py perf-results/1.10.6.json perf-results/1.11.0.json
```
...
1.10.6 and 1.11.0b2 the bundled compose gained:
...
places that disagree in 1.11.0b2 — the compose default says `2026.05.0-community`, but
