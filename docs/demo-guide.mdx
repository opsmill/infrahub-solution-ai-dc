...
Run the cascade again for **Fabric-B** (which uses Dell equipment) to build the second fabric.
...
1. In the Infrahub UI, navigate to the `demo` branch and click **Create Proposed Change**.
2. Give it a title (for example, *Build Fabric-A*) and submit.

The proposed change shows a diff of every object the Generators created — devices, IP allocations, links, and pools — against the `main` branch. This is the same review workflow used for any change in Infrahub.
