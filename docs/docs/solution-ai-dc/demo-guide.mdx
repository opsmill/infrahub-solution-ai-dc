...
- 6 single-vendor fabrics (Fabric-A Cisco, Fabric-B Arista, Fabric-C Dell, Fabric-D Juniper, Fabric-E SONiC, Fabric-F Cumulus), 18 pods, 48 racks
...
- Seed overlay tenants (`Blue` on Fabric-A, `Green` on Fabric-D, `Purple` on Fabric-E, `Amber` on Fabric-F) with VRFs and segments
...
- **Startup configuration** — a text artifact per device, rendered from that device's vendor template. Fabric-A is Cisco, so its devices render from the Cisco template; Fabric-B (Arista), Fabric-C (Dell), Fabric-D (Juniper), Fabric-E (SONiC) and Fabric-F (Cumulus) render from theirs. See [Multivendor configuration](./multivendor-config).
...
- **Multivendor rendering** — the same design produced Cisco, Arista, Dell, Juniper Junos, SONiC, and Cumulus Linux configuration, each device routed to its vendor's template
