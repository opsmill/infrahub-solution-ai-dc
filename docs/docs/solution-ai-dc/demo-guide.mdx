...
- **Startup configuration** — a text artifact per device, rendered from that device's vendor template. Fabric-A is Cisco, so its devices render from the Cisco template; Fabric-B (Arista), Fabric-C (Dell), Fabric-D (Juniper) and Fabric-E (SONiC) render from theirs. See [Multivendor configuration](./multivendor-config).
