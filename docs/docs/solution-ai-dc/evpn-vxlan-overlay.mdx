...
The leaf overlay block below is the **Cisco NX-OS** example (the Arista, Dell and Juniper templates render the equivalent constructs in their own syntax). The shape (see the [configuration artifact contract](https://github.com/opsmill/infrahub-solution-ai-dc/blob/main/specs/001-evpn-overlay/contracts/config-artifact.md) for the full definition):
...
- [Multivendor configuration](./multivendor-config) — how the overlay (and underlay) configuration is rendered per vendor (Cisco/Arista/Dell/Juniper).
