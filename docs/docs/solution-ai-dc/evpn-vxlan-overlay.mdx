...
Configuration is rendered **per vendor**: each device is routed to its vendor's template (Cisco NX-OS, Arista EOS, Dell OS10, Juniper Junos, or SONiC's `config` CLI + FRR), so the same overlay model produces the correct syntax for whichever fabric a device belongs to. In the demo data each fabric is single-vendor — Fabric-A Cisco, Fabric-B Arista, Fabric-C Dell, Fabric-D Juniper, Fabric-E SONiC. See [Multivendor configuration](./multivendor-config) for how vendor is resolved and rendered.

The leaf overlay block below is the **Cisco NX-OS** example (the Arista, Dell, Juniper and SONiC templates render the equivalent constructs in their own syntax). The shape (see the [configuration artifact contract](https://github.com/opsmill/infrahub-solution-ai-dc/blob/main/specs/001-evpn-overlay/contracts/config-artifact.md) for the full definition):
...
- [Multivendor configuration](./multivendor-config) — how the overlay (and underlay) configuration is rendered per vendor (Cisco/Arista/Dell/Juniper/SONiC).
