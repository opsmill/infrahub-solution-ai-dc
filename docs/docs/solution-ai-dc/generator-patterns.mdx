...
| `vendors.py` | `vendor_group_for_manufacturer`, `vendor_group_for_template` — resolves a device's vendor group (`cisco_devices` / `arista_devices` / `dell_devices`) for per-vendor artifact targeting |
