...
**Other settings.** `INFRAHUB_MCP_LOG_LEVEL` (default `info`) sets how much the container logs, readable with `docker compose logs infrahub-mcp`. `INFRAHUB_MCP_READ_ONLY=true` unmounts the write tools, so agents can query the graph but not change it.
