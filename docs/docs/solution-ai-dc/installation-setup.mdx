...
1. `inv publish-upstream` — mirrors every local branch into the gitignored bare repo `.upstream.git/`, plus the current checkout as `local`
...
4. `infrahubctl object load triggers.yml` — trigger rules, once the repository import has completed
...
Between steps 3 and 4, `inv load` blocks until Infrahub reports the repository `in-sync` at the commit that was just published — trigger rules reference Generator definitions that the import has to register first. Verify at any time with `infrahubctl repository list`.

:::note Publishing local commits
The containers clone the bare mirror, not your working tree — this is what makes a `git worktree` checkout work, since its `.git` is a file pointing outside the bind mount. All of your local branches are copied into the mirror, and the current checkout is published a second time under the fixed name `local`. `local` is the repository's `default_branch`, so Infrahub's `main` tracks whatever branch you have checked out on the host. Only committed work is published: after committing, re-run `inv publish-upstream` (or `inv load`) so the containers see it.
:::
...
| `repository.yml`              | `CoreRepository` object pointing to `/upstream/.upstream.git` (bare mirror), branch `local` |
...
| Publish     | `inv publish-upstream` | Mirror all local branches into `.upstream.git/`, checkout as `local` |
