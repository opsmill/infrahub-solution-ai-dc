...
| File                                                            | Purpose                                                                |
|-----------------------------------------------------------------|------------------------------------------------------------------------|
| `transforms/startup_config.gql` + `templates/startup_config_{cisco,arista,dell,juniper,sonic,cumulus}.j2` | Six per-vendor Jinja2 Transforms, each producing a startup configuration artifact for the devices in its vendor group (`{manufacturer}_devices`). See [Multivendor configuration](./multivendor-config). |
...
| File                             | Contents                                                                                                     |
|----------------------------------|--------------------------------------------------------------------------------------------------------------|
| `objects/01_groups.yml`          | `CoreStandardGroup` objects: `halls`, `racks`, `fabrics`, `pods`, `devices`, `tenants`, and the per-vendor device groups `cisco_devices` / `arista_devices` / `dell_devices` / `juniper_devices` / `sonic_devices` / `cumulus_devices` |
| `objects/02_manufacturer.yml`    | Equipment manufacturers (Cisco, Arista, Dell, Juniper, SONiC, Cumulus)                                      |
...
| `objects/10_fabric.yml`          | Fabric and Pod objects: Fabric-A (Cisco), Fabric-B (Arista), Fabric-C (Dell), Fabric-D (Juniper), Fabric-E (SONiC), Fabric-F (Cumulus) — each single-vendor |
| `objects/11_rack.yml`            | Rack objects across all six fabrics with leaf counts and per-vendor template assignments                    |
| `objects/12_overlay.yml`         | Seed overlay tenants `Blue` (Fabric-A), `Green` (Fabric-D), `Purple` (Fabric-E), and `Amber` (Fabric-F), each with a VRF and segments (routed, rack-scoped, and L2-only examples) |
