...
1. `inv publish-upstream` — mirrors every local branch into the bare repository `.upstream.git/` (ignored by Git), then force-pushes the current checkout over the mirror's `main`
2. `infrahubctl object load repository.yml` — registers the Git repository with Infrahub
3. `infrahubctl object load data/permissions.yml` — sample operator persona (account `john`, role and group definitions)
4. `inv wait-for-repository` — blocks until Infrahub reports the repository `in-sync` at the commit just published
5. `infrahubctl object load triggers.yml` — trigger rules, which reference Generator definitions the import has to register first

Everything else comes from the repository itself: `.infrahub.yml` declares `schemas`, `objects` and `menus` alongside the queries, Generators, transforms and artifact definitions, so Infrahub imports all of it during the repository sync. There is no separate schema, menu or object load step.

:::note Publishing local commits
The containers clone the bare mirror, not your working tree — this is what makes a `git worktree` checkout work, since its `.git` is a file pointing outside the bind mount. All of your local branches are copied into the mirror, and the current checkout is then force-pushed over the mirror's `main`. Infrahub imports the remote branch whose name matches its own, so its `main` follows whatever branch you have checked out on the host — note this means the mirror's `main` is your checkout, not the host's `main` branch. Only committed work is published: after committing, re-run `inv publish-upstream` (or `inv load`) so the containers see it.
:::
...
`data/tenant-red.yml` is intentionally kept out of the default load: a second overlay tenant (`Red` on Fabric-A) used to demonstrate scoped, non-clobbering day-two multi-tenant generation. Load it manually when you want it.
...
Imported in numbered order during the repository sync (`objects:` in `.infrahub.yml`):
...
Objects are imported from the repository (`objects:` in `.infrahub.yml`), not loaded directly. Trigger rules live in the top-level `triggers.yml` (not under `objects/`) and are loaded by `inv load` only once the repository import has completed. `data/tenant-red.yml` is loaded manually — see [Load supplementary data](#load-supplementary-data-optional).
...
| `repository.yml`              | `CoreRepository` object pointing to `/upstream/.upstream.git` (bare mirror), branch `main` |
| `tasks.py`                    | Invoke task definitions for environment management                                          |
| `.infrahub.yml`               | Central configuration wiring schemas, objects, menus, Generators, Transforms, queries, and artifacts |
| `triggers.yml`                | Trigger rules, loaded by `inv load` after the repository import completes                   |
...
| Load        | `inv load`        | Publish the repository, then load permissions and trigger rules    |
| Load schema | `inv load-schema` | Push schema files straight to Infrahub, bypassing the repository   |
| Load menu   | `inv load-menu`   | Push menu definitions straight to Infrahub, bypassing the repository |
| Publish     | `inv publish-upstream` | Mirror all local branches into `.upstream.git/`, checkout over `main` |
| Wait        | `inv wait-for-repository` | Block until Infrahub imported the published commit          |
...
- The containers clone `/upstream/.upstream.git` — make sure `inv publish-upstream` ran and that your latest commits were pushed to it; check `docker compose logs task-worker` for import errors
