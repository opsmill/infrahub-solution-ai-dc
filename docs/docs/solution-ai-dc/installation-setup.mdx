...
`INFRAHUB_USERNAME` and `INFRAHUB_PASSWORD` are used by `infrahubctl` to authenticate with the Infrahub API. `INFRAHUB_API_TOKEN` is the token an MCP client presents to the [Infrahub MCP server](#infrahub-mcp-server). `VERSION` sets the Infrahub image tag. If not using direnv, source the file manually or export these variables:
