...
On top of this physical fabric, a fourth Generator — the **OverlayGenerator** (registered as `generate-tenant`) — turns tenant intent into multi-tenant EVPN/VXLAN services. See [EVPN/VXLAN overlay](./evpn-vxlan-overlay).

The demo data includes six complete, single-vendor fabrics:

- **Fabric-A** — Cisco equipment
- **Fabric-B** — Arista equipment
- **Fabric-C** — Dell equipment
- **Fabric-D** — Juniper equipment
- **Fabric-E** — SONiC (Broadcom-chipset switches spanning three Tomahawk generations)
- **Fabric-F** — Cumulus Linux (NVIDIA Spectrum-ASIC switches spanning three Spectrum generations)

Each fabric is built entirely from one vendor's device templates, and each device's configuration is rendered from that vendor's template. See [Multivendor configuration](./multivendor-config).
...
| File | Contents |
|------|----------|
| `logical_design.yml` | NetworkFabric and NetworkPod — the design hierarchy with Generator signaling attributes and the fabric overlay ASN |
| `physical_location.yml` | LocationHall and LocationRack — physical locations with Generator target support |
| `device.yml` | NetworkDevice, NetworkInterface, and NetworkLink — devices with computed attributes, ASN, route-reflector role, and VTEP loopback |
| `ipam.yml` | IpamIPPrefix with role-based allocation and IpamIPAddress |
| `overlay.yml` | NetworkTenant, NetworkVrf, and NetworkSegment — the overlay intent model |
| `routing.yml` | NetworkBGPSession — the stored iBGP EVPN control plane |
...
|-----------|---------------|
| FabricGenerator | IP pool and overlay ASN allocation, super spine switch creation |
| PodGenerator | Spine switch creation, spine-to-super-spine cabling, iBGP EVPN sessions |
| RackGenerator | Leaf switch creation, leaf-to-spine cabling, VTEP loopbacks |
...
- **Startup configuration** — six per-vendor Jinja2 templates (Cisco, Arista, Dell, Juniper, SONiC, Cumulus), each producing a text/plain artifact for the devices in its vendor group
...
- Six complete single-vendor fabrics (Cisco, Arista, Dell, Juniper, SONiC, Cumulus) with pods, racks, device templates, and IP pools
...
→ [GitHub repository](https://github.com/opsmill/infrahub-solution-ai-dc)
