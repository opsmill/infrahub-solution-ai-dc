...
description: Implementation patterns used in the AI/DC solution Generators — queries, device creation, IP allocation, cabling, and cascade propagation.
---

# Generator patterns

This page covers the implementation patterns used in the AI/DC solution's Generators. It is aimed at developers who want to understand, adapt, or extend the code. Each section focuses on a specific pattern with annotated code from the bundle.

See [Modular Generator architecture](./modular-generator-architecture) for the architectural concepts (cascade mechanism, validation gates, interface allocation design) and [Design-driven automation](./design-driven-automation) for what the Generators read and produce.
...
The `if rack.checksum.value != checksum` guard prevents unnecessary saves — if the checksum has not changed (because the Generator produced identical results), no trigger fires and the cascade stops.

## Shared library

The Generators share common logic through the `solution_ai_dc` package (in `src/solution_ai_dc/`). This package is installed into the custom Infrahub Docker image so that task workers can import it at runtime.
...
- [Modular Generator architecture](./modular-generator-architecture) — cascade mechanism, validation gates, interface allocation design
