...
description: Prerequisites, environment setup, and repository structure for the AI/DC solution bundle.
---
...
- **Python 3.11+** (3.12 recommended — matches `.python-version`)
...
This installs all Python dependencies including `infrahub-sdk` (which provides `infrahubctl`), `invoke` (task runner), and the solution's own `solution-ai-dc` package.

### Configure environment variables

The repository includes a `.envrc` file (compatible with [direnv](https://direnv.net/)):
...
This downloads the base `docker-compose.yml` from `https://infrahub.opsmill.io` (if not already present), then runs `docker compose up -d`. The override file (`docker-compose.override.yml`) replaces the standard Infrahub image with a custom build that includes the solution's Python package.
...
This creates `CoreGeneratorAction` and `CoreNodeTriggerRule` objects for the automatic cascade. The trigger file uses a `.save` extension so it is excluded from the initial `inv load` — triggers must be loaded after the repository has synced, because they reference Generator definitions that need to exist first.
...
| File | Contents |
|------|----------|
| `schemas/logical_design.yml` | `NetworkFabric` and `NetworkPod` — the design hierarchy with cascade attributes (checksum, amount_of_spines) |
...
| `schemas/generator.yml` | `GeneratorTarget` generic enabling trigger-based cascades via checksum attribute |
...
| File | Purpose |
|------|---------|
| `Dockerfile` | Builds a custom Infrahub image with the `solution-ai-dc` Python package installed |
...
| `.envrc` | Environment variables for `infrahubctl` authentication |
...
For modifying Generator code or adding new Transforms:
...
The `Dockerfile` extends the base Infrahub image (`registry.opsmill.io/opsmill/infrahub`) with the solution's Python package. Running `inv build` builds the image locally; `inv restart` picks up the new build. The `docker-compose.override.yml` also mounts `src/` into the task worker container, so changes to utility code in `src/solution_ai_dc/` are picked up without rebuilding.
...
### Generator fails with prerequisite error

- "the fabric doesn't seem to be fully generated yet" — the PodGenerator requires all super spines to exist before it runs. Run the FabricGenerator first.
- "the pod doesn't seem to be fully generated yet" — the RackGenerator requires all spines to exist. Run the PodGenerator for that pod first.
- These errors are by design: each Generator validates that the tier above is complete before proceeding.

### Cascade stops mid-way

- Check task execution in the Infrahub UI for failed tasks
- Triggers fire only on non-main branches (`branch_scope: other_branches`) — verify you are working on a branch
- Triggers must be loaded after repository sync — if loaded before, Generator action references will not resolve
