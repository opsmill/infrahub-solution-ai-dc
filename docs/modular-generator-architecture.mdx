...
The AI/DC solution uses three Generators — FabricGenerator, PodGenerator, RackGenerator — that chain automatically through a checksum-based cascade. This page explains the cascade mechanism, how each tier validates that the previous tier completed, and how interface allocation works without Infrahub's resource allocators.
...
### Checksum-driven chaining

Each Generator writes a checksum to its child objects after it finishes. That checksum change fires a trigger rule, which runs the next Generator tier. The chain repeats until the lowest tier completes.
...
The checksum trigger handles the cascade flow. The additional attribute triggers allow a Generator to re-run when an operator changes a design parameter (for example, updating `amount_of_spines` on a Pod).
