...
- [X] T032 [P] Update `AGENTS.md` — add `clusters.py` to the Core Library list ("Cluster peering: turns a cluster's members into ordered Cilium peering records; eligibility and deterministic ordering; pure, no client"), add `cilium_manifest.py` to the Transforms section, and note the new `queries/` directory and the `application/yaml` artifact. `AGENTS.md` is the single source of truth for repository structure and is currently silent on all of it — **done**. Four edits: `clusters.py` in Core Library, `cilium_manifest.py` in Transforms (noting that selection/ordering live in `clusters.py`, and that `.infrahub.yml` wires it to an `application/yaml` artifact named `Cilium BGP Manifest` targeting `kubernetes_clusters`), a new `queries/` bullet in Data Files describing `artifact_ids.gql`/`ArtifactIDs` and the `/api/query/ArtifactIDs` path, and `kubernetes.yml` added to the existing `schemas/` enumeration — that last one beyond the task's letter, but the bullet already names `overlay.yml` and `routing.yml`, so documenting the artifact while omitting the schema defining its target kind would have been the same gap. The "Five generators" heading is deliberately untouched: this feature adds none (ADR-0002 boundary)
- [X] T033 [P] Add glossary entries to `CONTEXT.md` for **Kubernetes cluster**, **cluster member**, **node selector**, **Server service** and **Server**. Verified: `CONTEXT.md` currently has no entry for any of these terms — the source PRD flagged this as pending documentation with a draft written alongside it and never applied. Since `CONTEXT.md` is the project's domain-language source of truth, leaving five new domain terms out of it is a real gap — **done**. Two new `###` sections after "EVPN constructs": **Server attachment** (Server, Server service — feature 003's terms, missing until now) and **Kubernetes clustering** (Kubernetes cluster, cluster member, node selector), each entry following the house shape of a bold term, a definition in domain rather than implementation language, and an `_Avoid_:` line. **Cluster member** is stated explicitly as a name for a role and **not** a stored entity. Three bullets were also added to `## Relationships` so the new terms are structurally placed like every other term, including that an L3 member's stored **BGP sessions** are the one source both the leaf config and the manifest render from. Note the file's opening paragraph still describes the solution as "fabric + EVPN/VXLAN overlay" and does not mention server attachment or clustering — left alone as outside this task, but it is now understated
- [X] T034 [P] Record the multi-cluster bound where an implementer will actually hit it — a comment in `.infrahub.yml` beside the `cilium_bgp_manifest` artifact definition noting that all artifacts of one definition share one name, that Vidra selects by name only and syncs every match, and that v1 therefore supports one Cilium-consuming cluster per Infrahub branch (critique finding P1; full reasoning in `spec.md` Assumptions and `contracts/cilium-manifest-artifact.md`) — **done** as a second comment paragraph below the existing `artifact_name` note, which is left intact: the two say different things (that note is "never rename this"; this one is "this name is shared by every artifact of the definition"). It cites `cabling_plan` directly above it as the in-repo proof that one definition's artifacts all share one name, and states the bound as rendering-correct-but-delivery-ambiguous so a reader does not go looking for a rendering bug
- [X] T035 Consider whether an ADR is warranted for the one-cluster-config-per-member decision and record it in `dev/adr/` if so — the decision rests on a verified external constraint (`peerAddress` is not expressible in `CiliumBGPNodeConfigOverride`, `research.md` R1) that a future reader would otherwise have to re-derive, which is exactly the ADR criterion the existing `dev/adr/0001`–`0006` follow. If judged not warranted, note why and move on rather than writing a thin record — **judged warranted; written** as `dev/adr/0007-one-cluster-config-per-cluster-member.md`. It clears the bar the existing records set, on the same three counts ADR-0006 does: it has a real rejected alternative (one shared cluster config + one `CiliumBGPNodeConfigOverride` per member), that alternative is rejected on a *verified external* constraint rather than a preference (`CiliumBGPNodeConfigOverride` exposes `localASN`/`localAddress`/`localPort`/`routerID` but no `peerAddress`), and the decision fixes an interface others build on — the N + 2 document count and ordering that the artifact contract and the consumer's checksum comparison both depend on. Without the record, the next reader who notices the manifest repeats a near-identical document per member has to re-derive the answer from Cilium's docs to learn it is not redundancy. `research.md` R1 holds the same reasoning but is feature-scoped working material; `dev/adr/` is where this repo keeps decisions that outlive their spec directory. `dev/adr/README.md` documents only the naming convention and carries no index, so no registration was needed; `CLAUDE.md`'s `dev/adr/0001`–`0006` range was bumped to `0007`, since adding the record is what made it stale
- [X] T036 Run `inv lint` and `inv format` and confirm clean — `ruff` with `select = ["ALL"]`, `mypy` strict, `yamllint` at 140 chars, and `rumdl` over the new markdown — **done, clean**: `inv format` "47 files left unchanged / All checks passed!", then `inv lint` gave rumdl "No issues found in 77 files", yamllint silent, ruff "All checks passed!", mypy "Success: no issues found in 47 source files". One real catch on the way: rumdl `MD049` flagged a `*for*` in the new `CONTEXT.md` prose, because its emphasis style is per-file consistency and that file uses `_underscore_` throughout — fixed rather than disabled. `dev/adr/` is **not** in rumdl's exclude list, so ADR-0007 was linted too
