...
- [X] T005 Local validation gate — all four must pass: `docker compose config >/dev/null`, `python -m json.tool .mcp.json >/dev/null`, `uv run inv lint`, `uv run inv test` (depends on T002, T003, T004)
