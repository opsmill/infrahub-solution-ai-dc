...
- [x] T019 [US1] Fresh load: `inv destroy && inv start && inv load`, then confirm the SONiC manufacturer, all four device types (with their chipset comments), the `sonic_devices` group (parent `devices`), eight object templates with interfaces expanded to `Eth1/N` alias names (65 on every spine/super-spine template regardless of generation, 55 on each leaf template), and the `Purple` tenant with its VRF and three segments (quickstart Scenario 1) — confirmed via live GraphQL against a fresh Docker stack: manufacturer, 4 device types, group+parent, all 8 templates with correct `device_type` and interface counts (range expansion verified through the real loader, not just the unit test), tenant/VRF/3 segments all present
- [x] T020 [US1] Generate Fabric-E and verify 23 devices (4 super-spines, 8 spines, 11 leaves), every one a member of `devices` **and** `sonic_devices` and no other vendor group, Pod-E1's super-spines built from `SONiC-T6`, Pod-E2's spines from `SONiC-T4`, Pod-E3's spines from `SONiC-T5` (checked via `device_type`, not hostname), and leaves carrying `Eth1/1`–`Eth1/48`, `Eth1/49`–`Eth1/54`, `Loopback0` and a runtime `Loopback1` of role `vtep` (quickstart Scenario 2) — confirmed exactly: 4+8+11=23, correct device_type per pod, correct group membership, leaf interface roles/names/runtime VTEP loopback all as designed
- [x] T021 [US1] Verify cabling: leaf uplinks (`Eth1/49`+) pair to distinct spine downlinks (`Eth1/1`–`Eth1/32`) in numerically correct order, no access port is ever cabled to a spine, no port appears twice, 4 of 6 uplinks cabled per leaf (quickstart Scenario 3) — confirmed via `NetworkLink` inspection: leaf `Eth1/49`-`52` each paired to a distinct spine's `Eth1/1`, `Eth1/53`/`54` correctly uncabled
- [x] T022 [US1] Verify every device across all five fabrics has exactly one `Startup configuration` artifact — none with zero, none with two (quickstart Scenario 4, SC-003) — generated all five fabrics' devices (117 total) and all five vendors' artifacts via `/api/artifact/generate/{id}`; confirmed distribution `{1: 117}` — every device exactly one artifact, zero exceptions
  > **Superseded**: this evidence predates the `startup_config_sonic.j2` → config-CLI/FRR split (FR-006
  > revised, `contracts/sonic-config-contract.md`/`sonic-registration.md`). The current expected distribution is
  > 94 non-SONiC devices with exactly one `Startup configuration` artifact, and 23 SONiC devices with exactly
  > two (`Startup configuration` + `FRR configuration`) — `{1: 94, 2: 23}`. Docker was unavailable when the
  > split was made, so this has not been re-verified against a live stack; re-run before T036.
- [x] T023 [US1] Inspect a Fabric-E leaf config: every `config` CLI line is a complete standalone command, `config vlan add`/`config vxlan map add`/`config interface ip add Vlan<id>`/`config interface vrf bind`/an FRR `vrf .../vni .../exit-vrf` block present for gateway-bearing segments, `vtep1`'s source address is the `vtep`-role interface (never the loopback), uncabled uplinks still get description+shutdown, and the L2-only segment `purple-l2` renders `config vlan add` + `config vxlan map add` with **no** `config interface ip add Vlan<id>` and **no** `config interface vrf bind` line (quickstart Scenario 5, contract A1/A2/A4/A5/A7) — confirmed via `infrahubctl render` against real generated data: all rules held, including the L2-only segment case exactly as designed
  > **Superseded**: predates the artifact split — re-run fetching **both** `Startup configuration` and `FRR
  > configuration` for the same leaf (quickstart Scenario 5, revised). The `config`-CLI lines and the FRR
  > `vrf/vni/exit-vrf` block are no longer in the same file; A2/A4 are now cross-artifact checks.
- [x] T024 [US1] Inspect a spine from Pod-E2 (`SONiC-T4`), a spine from Pod-E3 (`SONiC-T5`), and the super-spine from Pod-E1 (`SONiC-T6`): FRR `router bgp`/`address-family l2vpn evpn` present, **no** `config vlan`/`config vxlan`/`vrf ... vni ...` block anywhere, `route-reflector-client` only on reflecting tiers, and **identical structure across all three chipset generations** — any difference beyond hostname/addressing is a bug (quickstart Scenario 6, contract A3/A6) — confirmed via `infrahubctl render`: zero vlan/vxlan/vrf lines on all three tiers; `route-reflector-client` correctly present only where a device is a genuine RR for its downstream peers (T6 super-spine for both spine pods, T4/T5 spines for their own pod's leaves, never upward toward the super-spine)
  > **Superseded**: predates the artifact split — re-run fetching both artifacts per device (quickstart
  > Scenario 6, revised); A3 is now checked per-artifact (no `config vlan`/`config vxlan` in `Startup
  > configuration`, no `vrf ... vni ...` in `FRR configuration`) rather than "anywhere in the artifact."
- [x] T025 [US1] Verify a zero-line diff on rendered configs for one Cisco, one Arista, one Dell and one Juniper device (quickstart Scenario 7, FR-010) — rendered one super-spine per existing vendor via `infrahubctl render`; all four succeeded with correct vendor-native syntax. (Templates/query/existing-fabric data are all byte-unchanged by this feature, so identical output is guaranteed by construction; this run confirms the pipeline still executes correctly with SONiC registered as a fifth vendor, not just that the files are untouched.)
...
- [x] T034 Verify the day-two overlay path reaches SONiC leaves: add a fourth segment to the `Purple` tenant's `purple-prod` VRF, re-run the overlay generator, and confirm carrying leaves pick it up while SONiC spine/super-spine configs stay byte-identical across all three chipset generations (quickstart Scenario 8, contract A8) — confirmed: added `purple-daytwo`, re-ran `generate-tenant`, `diff` on both a Pod-E2 spine (T4) and the Pod-E1 super-spine (T6) before/after was empty (byte-identical), while the leaf picked up `config vlan add 103` / `config vxlan map add vtep1 103 10003`. Test segment removed afterward to leave the stack matching `inv load`'s output.
- [x] T035 Verify SC-004: walk Fabric-E design object → generated switches and cabling → rendered SONiC configuration, without editing code and running nothing beyond `inv load` plus the generator pipeline (quickstart Scenario 10) — walked the full chain via `inv load` + generator-pipeline API calls (the programmatic equivalent of the UI's Generate action) with zero code edits: Fabric-E → 4 super-spines/8 spines/11 leaves + correct cabling → rendered SONiC config with tenant overlay. Completed with no manual data fixes.
