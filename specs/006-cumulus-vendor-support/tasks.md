...
- [X] T018 [US1] Run `inv lint` and `inv test`; both must pass before loading (confirmed: `uv run inv lint` — rumdl/yamllint/ruff/mypy all clean, 0 issues; `uv run pytest tests/unit -q` — 311 passed; `uv run pytest tests/integration -q` — 25 errors, all pre-existing "Docker daemon not available" failures unrelated to this feature, same class of failure T018 in specs/005-sonic-vendor-support/tasks.md recorded for the identical reason)
- [ ] T019 [US1] **BLOCKED — no Docker daemon in this sandbox.** Fresh load requires `inv destroy && inv start && inv load` against a live Infrahub stack; `docker info` fails here. Object-data correctness (manufacturer, four device types, group, eight templates' interface expansion, Amber tenant/VRF/segments) is covered instead by T008's unit test and manual review of the committed YAML against data-model.md — but the live-load confirmation itself did not run. Re-run in an environment with Docker, mirroring specs/005-sonic-vendor-support's own T019 which was originally blocked the same way and closed out later in a follow-up session (commit fe54a92) once a live stack was available.
- [ ] T020 [US1] **BLOCKED — same Docker unavailability as T019.** Requires generating Fabric-F against a live stack to confirm device counts, vendor-group membership, per-pod device_type, and leaf interface naming.
- [ ] T021 [US1] **BLOCKED — same Docker unavailability as T019.** Requires a live stack's `NetworkLink` objects to verify cabling order.
- [ ] T022 [US1] **BLOCKED — same Docker unavailability as T019.** Requires generating artifacts for all six fabrics' devices against a live stack.
- [ ] T023 [US1] **BLOCKED — same Docker unavailability as T019** for the live-artifact form of this check. Partial substitute performed instead: the T009-T017 subagent's synthetic Jinja2 render (mock data matching the shared query's shape) exercised this exact leaf scenario and confirmed A1/A2/A4/A5/A7 against the rendered output — see commit ef6a3f2's report. That is evidence the template is *structurally* correct, not a substitute for inspecting a real generated artifact.
- [ ] T024 [US1] **BLOCKED — same Docker unavailability as T019** for the live-artifact form of this check. Partial substitute: the same synthetic render exercised a spine/RR mock device and confirmed A3/A6. Real per-generation (Spectrum-2/3/4) artifacts were not compared against each other since none were generated.
- [ ] T025 [US1] **BLOCKED — same Docker unavailability as T019.** Requires rendering existing-vendor artifacts against a live stack to diff before/after. Indirect evidence only: `git diff` on `transforms/templates/startup_config_{cisco,arista,dell,juniper,sonic}.j2`, `transforms/startup_config.gql`, and every non-Fabric-F object-data file is empty (this feature's changes to `objects/10_fabric.yml`/`11_rack.yml`/`12_overlay.yml` are pure appends after the existing fabrics' blocks), so byte-identical rendering follows by construction — but this was not confirmed by actually re-rendering.
...
- [ ] T034 **BLOCKED — no Docker daemon in this sandbox** (same as T019-T025). Requires a live stack to add a day-two segment and re-generate.
- [ ] T035 **BLOCKED — no Docker daemon in this sandbox** (same as T019-T025). Requires a live stack to walk the full generation pipeline.
