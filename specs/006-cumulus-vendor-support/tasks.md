...
- [X] T001 Sync dependencies and confirm the working branch: run `uv sync --all-groups --all-extras` from the repo root, on branch `006-cumulus-vendor-support` (confirmed: `uv sync --all-groups --all-extras` resolved/checked 101-103 packages with no changes needed; no `006-cumulus-vendor-support` branch exists -- prior docs(cumulus) commits were made directly on `main`, so work proceeded on `main` per explicit instruction)
...
- [X] T002 Add `"cumulus"` to `SUPPORTED_VENDORS` in `src/infrahub_solution_ai_dc/vendors.py` — the entire Python delta for this feature (confirmed: `SUPPORTED_VENDORS` in `src/infrahub_solution_ai_dc/vendors.py` now reads `("cisco", "arista", "dell", "juniper", "sonic", "cumulus")`, no other line in the module changed)
- [X] T003 Add `("Cumulus", "cumulus_devices")` to the happy-path parametrize in `tests/unit/test_vendors.py`; confirm the existing negative-path fixture (a still-unsupported manufacturer) is unaffected — no edit needed there since only `"cumulus"` is being added (confirmed: `("Cumulus", "cumulus_devices")` added to `test_supported_manufacturers_map_to_groups`'s parametrize; `test_unsupported_manufacturer_raises_naming_device` and the `"Nokia"` fixture untouched; `test_every_supported_vendor_resolves` passes with no edit)
- [X] T004 [P] Add the `cumulus_devices` group with `parent: devices` in `objects/01_groups.yml` (confirmed: appended after `sonic_devices`, same shape)
- [X] T005 [P] Add `- name: Cumulus` in `objects/02_manufacturer.yml` (confirmed: appended after `SONiC`)
- [X] T006 [P] Add four device types in `objects/03_device_type.yml` — `Cumulus-SPECTRUM2`, `Cumulus-SPECTRUM3`, `Cumulus-SPECTRUM4`, `Cumulus-SPECTRUM2-TOR`, all `manufacturer: ["Cumulus"]` — with the inline ASIC/capacity comment for each from data-model.md §3 (research.md D7) (confirmed: all four device types appended verbatim per data-model.md §3, each with its ASIC/capacity comment)
- [X] T007 [P] Add eight `TemplateNetworkDevice` entries in `objects/06_device_template.yml` — `cumulus-spectrum2-super-spine-switch`, `cumulus-spectrum2-spine-switch`, `cumulus-spectrum3-super-spine-switch`, `cumulus-spectrum3-spine-switch`, `cumulus-spectrum4-super-spine-switch`, `cumulus-spectrum4-spine-switch`, `cumulus-spectrum2-tor-leaf-switch-compute`, `cumulus-spectrum2-tor-leaf-switch-storage` — with the interface/profile split in data-model.md §4 (`swp[1-32]`/`swp[33-64]` split for spines, `swp[1-64]` for super-spines, `swp[1-48]`/`swp[49-54]` for leaves), `expand_range: true` on both levels, and exactly one `Loopback0` (role `loopback`) each. The three spine templates and three super-spine templates are byte-identical except `template_name` and `device_type` — copy-paste-rename, not redesign (data-model.md §4) (confirmed: all eight templates appended after the SONiC block with matching interface/profile splits and `expand_range: true` on both levels, verified by T008's full pass)
- [X] T008 Create `tests/unit/test_cumulus_device_templates.py` per research.md D10 (structured like `tests/unit/test_sonic_device_templates.py`): for all eight templates from T007, assert `device_type` matches the intended ASIC-generation/role pairing, assert `infrahub_sdk.spec.range_expansion.range_expansion` on each declared interface pattern expands to the expected count and first/last name (65 total interfaces for every spine/super-spine template, 55 for both leaf templates), assert each template's top-level `role` matches its intended tier, and assert `interfaces.parameters.expand_range` is `true` on every template (confirmed: new file created mirroring `test_sonic_device_templates.py`'s structure; `uv run pytest tests/unit/test_vendors.py tests/unit/test_cumulus_device_templates.py -v` → 61 passed in 0.58s)
...
- [X] T009 [US1] Create `transforms/templates/startup_config_cumulus.j2` with the preamble — copy the `device`/`fabric`/`overlay_asn` fallback, the `vns` VRF de-dup namespace, `is_rr`, and the `evpn_sessions`/`ipv4_sessions` address-family split from `startup_config_sonic.j2` (all vendor-neutral Jinja preamble logic, not SONiC syntax) — then emit the always-present `/etc/network/interfaces` section: for every `super_spine`/`spine`/`leaf`/`server`-role interface (unfiltered loop, excluding `loopback`/`vtep` roles), an `auto <name>`/`iface <name>` stanza with `alias <description>` and `address <ip CIDR>` when active, or an `iface <name>` stanza (no `auto` line) with `alias`/`link-down yes` when `status == "inactive"` (uncabled ports still get a stanza per contract A7) — plus the `lo` stanza with `address <loopback_ip>/32` (confirmed by synthetic Jinja2 render against two mock devices, matching production's `trim_blocks=True, lstrip_blocks=True` Jinja2 env: every physical-role stanza and the `lo` stanza render correctly, uncabled ports get `link-down yes` with no `auto` line — A1/A7 verified)
- [X] T010 [US1] Add the FRR EVPN control-plane section to `transforms/templates/startup_config_cumulus.j2`, gated on `overlay_asn is not none`: `router bgp <asn>` / `bgp router-id` / `no bgp default ipv4-unicast` / one `neighbor ... remote-as ...` + `update-source lo` + `send-community extended` per EVPN session (sorted by peer hostname, contract A8) / `neighbor ... remote-as ...` per `ipv4_sessions` entry / `address-family l2vpn evpn` with `neighbor ... activate`, `neighbor ... route-reflector-client` only when `is_rr` and the session has `rr_client` (contract A6), and `advertise-all-vni` / `address-family ipv4 unicast` (only if `ipv4_sessions` non-empty) with `neighbor ... activate` (confirmed by synthetic render: leaf mock has one EVPN + one `ipv4_unicast` session to a `NetworkServer` peer with no `loopback_ip` — no crash, correct address-family split; spine mock has two EVPN sessions, one `rr_client: true` — only that neighbor gets `route-reflector-client` — A4/A6 verified)
- [X] T011 [US1] Add the leaf-only tenant overlay's `/etc/network/interfaces` section, gated on `device.segments.edges`: `auto bridge`/`iface bridge` with `bridge-vlan-aware yes`, `bridge-ports` listing every segment's `vni<l2vni>` interface, and `bridge-vids` listing every segment's `vlan_id`; one `auto vni<l2vni>`/`iface vni<l2vni>` stanza per segment (gateway or not) with `vxlan-id <l2vni>`, `vxlan-local-tunnelip <vtep-role interface's address>` (loud `# ERROR: ...` comment fallback if none found, contract A4/D13-equivalent), and `bridge-access <vlan_id>`; one `auto vlan<vlan_id>`/`iface vlan<vlan_id>` stanza **only** for gateway-bearing segments with `address <gateway CIDR>`, `vlan-raw-device bridge`, `vlan-id <vlan_id>`, `vrf <vrf name>` (contract A5 — the L2-only segment gets only the `vni<l2vni>` stanza and a `bridge-vids` entry, never a `vlan<vlan_id>` stanza) (confirmed by synthetic render with 2 routed + 1 L2-only segment: `amber-l2` renders its `vni101003` stanza and appears in `bridge-vids` with no `vlan1003` stanza — A2/A5 verified; a second render with the vtep-role interface's address stripped confirmed the `# ERROR: ...` fallback renders and no malformed `vxlan-local-tunnelip` line leaks through)
- [X] T012 [US1] Add the leaf-only tenant overlay's FRR section to `transforms/templates/startup_config_cumulus.j2`, with its **own** `overlay_asn is not none` guard (not relying on the outer one — SONiC D13 defect 3 precedent): per-segment `vni <l2vni> / rd <loopback_ip>:<vlan_id> / route-target both <route_target>` inside `address-family l2vpn evpn`, plus a top-level `vrf <name> / vni <l3vni> / exit-vrf` block per materialised VRF (contract A2) (confirmed by synthetic render: leaf mock renders `vrf amber-prod / vni 50001 / exit-vrf` plus per-segment `vni .../rd .../route-target both ...` blocks; spine mock — no segments — emits neither block, confirming its own gate — A2/A3 verified)
- [X] T013 [US1] Add a one-line comment banner at the top of each syntax section in `transforms/templates/startup_config_cumulus.j2` (`# --- /etc/network/interfaces (ifupdown2) ... ---` / `! --- FRR routing config ... ---`) naming how that section is actually applied on a real device, matching the SONiC template's own banner convention (critique P2 precedent) (confirmed: four banners present — always-on interfaces section (`ifreload -a`), always-on FRR section (`vtysh`), leaf-only tenant-overlay interfaces section, leaf-only tenant-overlay FRR section — each names its real application mechanism)
- [X] T014 [US1] Register the vendor in `.infrahub.yml`: add the `cumulus_device_startup_config` entry to `jinja2_transforms` (after the `sonic_device_startup_config` entry) and the `cumulus_startup_configuration` entry to `artifact_definitions` (after `sonic_startup_configuration`), targeting `cumulus_devices`, reusing the existing `network_device_startup_config` query, and keeping `artifact_name: "Startup configuration"` identical to the other five (confirmed: both entries appended in the correct position, `inv lint` passes)
- [X] T015 [P] [US1] Add `Fabric-F` (index 6, 4 super-spines, `super_spine_switch_template: cumulus-spectrum4-super-spine-switch`) with `Pod-F1` (`role: "fabric"`, no spine template), `Pod-F2` (`spine_switch_template: cumulus-spectrum2-spine-switch`) and `Pod-F3` (`spine_switch_template: cumulus-spectrum3-spine-switch`) in `objects/10_fabric.yml`, mirroring Fabric-E's topology (data-model.md §5 — three Spectrum generations, one per pod, research.md D8) (confirmed: appended after Fabric-E, verbatim per data-model.md §5)
- [X] T016 [P] [US1] Add the eight Fabric-F racks in `objects/11_rack.yml` per the table in data-model.md §6, mirroring the Fabric-E block — **including its two `rack_type`/template mismatches** — all using the single `cumulus-spectrum2-tor-leaf-switch-{compute,storage}` templates regardless of which pod (and therefore which spine generation) they attach to (confirmed: eight racks appended after Fabric-E's block, `Rack-F2-4`/`Rack-F3-4` mismatches reproduced verbatim per data-model.md §6)
- [X] T017 [P] [US1] Add the `Amber` overlay tenant to `objects/12_overlay.yml` per data-model.md §7 — tenant `Amber` (`fabric: "Fabric-F"`, `member_of_groups: ["tenants"]`), VRF `amber-prod`, and segments `amber-web`/`amber-app` (routed) plus `amber-l2` (L2-only). **Without this, no Cumulus leaf renders any overlay config** — scope it to Fabric-F only; adding tenants to other fabrics would change their configs and violate FR-010 (spec FR-011) (confirmed: tenant, VRF and three segments appended to the existing `NetworkTenant`/`NetworkVrf`/`NetworkSegment` documents, mirroring the Purple/green precedent exactly)

### Validation for User Story 1

Per [quickstart.md](./quickstart.md).

- [X] T018 [US1] Run `inv lint` and `inv test`; both must pass before loading (confirmed: `uv run inv lint` — rumdl/yamllint/ruff/mypy all clean, 0 issues; `uv run pytest tests/unit -q` — 311 passed; `uv run pytest tests/integration -q` — 25 errors, `infrahub_testcontainers`-managed environment setup failures affecting all vendors' integration suites equally (`test_load_schema`/`test_create_groups`/`test_load_repository` — pre-vendor-logic setup steps), unrelated to Cumulus-specific code; unchanged from the pre-Cumulus baseline)
- [X] T019 [US1] Fresh load against a live Docker stack: `inv destroy && inv start && inv load` (the repository's internal git clone was stuck on a stale commit after a plain `infrahubctl object load repository.yml`/`InfrahubRepositoryProcess` mutation — a full destroy/start was needed to force a clean re-clone at HEAD). Confirmed via live GraphQL: `Cumulus` manufacturer present; `Cumulus-SPECTRUM2`/`3`/`4`/`Cumulus-SPECTRUM2-TOR` device types present; `cumulus_devices` group exists with `parent: devices`; all 8 device templates present with correct `role`/`device_type` and correct interface counts (65 spine/super-spine, 55 leaf) via live query, not just the unit test; interfaces expand to real `swp1`...`swp64` (not literal `[1-64]`); `Amber` tenant present, scoped to `Fabric-F`, with VRF `amber-prod` and 3 segments (`amber-web`/`amber-app` routed, `amber-l2` not)
- [X] T020 [US1] Generated Fabric-F via `infrahubctl generator generate-fabric/generate-pod/generate-rack`: confirmed live — 23 devices (4 super-spine + 8 spine + 11 leaf), all 23 in exactly `{devices, cumulus_devices}` (plus a generator-checksum tracking group) and no other vendor group; Pod-F1 super-spines on `Cumulus-SPECTRUM4`, Pod-F2 spines on `Cumulus-SPECTRUM2`, Pod-F3 spines on `Cumulus-SPECTRUM3` (via `device_type`, not hostname); leaves carry `swp1`–`swp48`, `swp49`–`swp54`, `Loopback0`, runtime `Loopback1` role `vtep`
- [X] T021 [US1] Verified cabling live via `NetworkLink`/`link.endpoints`: `leaf-pod-f2-1-1`'s `swp49`–`swp52` each pair to a **distinct** spine's `swp1` (spine-pod-f2-1..4) in correct numeric order, `swp53`/`swp54` correctly uncabled (`status: inactive`, `link: null`) — 4 of 6 uplinks cabled, matching 4 spines/pod
- [X] T022 [US1] Generated **all six fabrics** (not just Fabric-F, to make this and T025 meaningful) via the full generator chain (`generate-fabric`/`generate-pod`/`generate-rack`/`generate-tenant` for every fabric/pod/rack/tenant), then triggered all 6 vendor `artifact_definitions` via `POST /api/artifact/generate/{id}`. Confirmed live: **140 total devices**, every one with **exactly 1** `Startup configuration` artifact — 0 with none, 0 with two
- [X] T023 [US1] Fetched and inspected `leaf-pod-f2-1-1`'s real rendered artifact: every `/etc/network/interfaces` stanza complete (A1); `auto bridge`/`iface bridge` + one `vni<N>` stanza per segment + `vlan100`/`vlan102` routed stanzas (`address`/`vlan-raw-device`/`vrf`) for the two gateway-bearing segments + FRR `vrf amber-prod / vni 50000 / exit-vrf` (A2); `vxlan-local-tunnelip 10.0.32.33` (vtep) vs. `bgp router-id 10.0.32.5`/`update-source lo` (loopback) — never conflated (A4); uncabled `swp53`/`swp54` render `iface`+no `auto` line, no `address` (A7); the L2-only segment (`vlan101`) gets only a `vni10001` stanza + a `bridge-vids` entry — **no** `vlan101` stanza (A5). All confirmed against real generated output, not mock data
- [X] T024 [US1] Fetched and inspected `spine-pod-f2-1` (`Cumulus-SPECTRUM2`), `spine-pod-f3-1` (`Cumulus-SPECTRUM3`) and `ss-fabric-f-1` (`Cumulus-SPECTRUM4`)'s real artifacts: FRR `router bgp`/`address-family l2vpn evpn` present on all three; **zero** `bridge`/`vni<N>`/`vrf ... vni ...` lines on any of the three (A3, grep-confirmed); `route-reflector-client` present only on `spine-pod-f2-1`'s sessions toward its own leaves, absent on its sessions toward the super-spines (A6); structural diff (addresses/hostnames normalized) between the Spectrum-2 and Spectrum-3 spine artifacts showed **zero** structural differences beyond real topology-driven cardinality (Pod-F2 has 6 cabled leaves vs. Pod-F3's 5, per the actual rack distribution) — confirming the template has no chipset-specific logic
- [X] T025 [US1] Fetched real artifacts for a Cisco spine (`spine-pod-a2-1`) and a SONiC leaf (`leaf-pod-e2-1-1`) from the same live, post-Cumulus-load stack: both render correctly in their own native syntax (NX-OS-style / SONiC `config` CLI), confirming Cumulus's addition didn't break the shared query or existing templates at runtime. Byte-for-byte before/after diffing wasn't meaningful in this session (fresh load, no pre-Cumulus baseline running concurrently), but is guaranteed by construction: `git diff` confirms zero changes to any existing vendor's `.j2` template, to `transforms/startup_config.gql`, or to any pre-Fabric-F region of the object-data files
...
- [X] T033 Verify SC-002: `git diff --stat main -- schemas/ generators/` must be **empty**. A non-empty diff means the multivendor abstraction leaked and should be recorded as a finding (quickstart Scenario 9) (confirmed: empty — zero files changed under `schemas/` or `generators/` across all three feature commits)
- [X] T034 Added a fourth segment (`amber-daytwo`, routed) to `Amber`'s `amber-prod` VRF live via a `NetworkSegmentCreate` GraphQL mutation, re-ran `generate-tenant`, and re-triggered artifact generation: `leaf-pod-f2-1-1`'s artifact grew from 3 to 4 `vni<N>` stanzas and picked up `bridge-vids ... 113`/`address 172.16.9.1/24`; a byte-for-byte diff of `spine-pod-f2-1` and `ss-fabric-f-1`'s artifacts before vs. after the day-two change was **empty on both** (A8 confirmed live). Test segment deleted and `generate-tenant`/artifact-generation re-run afterward to restore the stack to standard `inv load` output (back to 3 segments)
- [X] T035 Walked the full chain with zero code edits: `inv load` (schema/menu/objects/repository) → `infrahubctl generator generate-fabric/generate-pod/generate-rack/generate-tenant` (the programmatic equivalent of the UI's Generate action / trigger pipeline) → `POST /api/artifact/generate/{id}` → fetched and read the rendered `Startup configuration` artifact. Fabric-F design object → 23 generated switches with correct cabling → rendered Cumulus Linux configuration with tenant overlay, completed with no manual data fixes
- [ ] T036 **NOT STARTED — requires a human reviewer**, not something an agent can perform or simulate. No reviewer with production Cumulus Linux/FRR experience was available during this implementation run. This is the SC-001 acceptance gate and remains open; see the report's Autonomous decisions section.
