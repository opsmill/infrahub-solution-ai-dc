...
async def provision_fabric_cascade(client: InfrahubClient, *, branch: str) -> None:
    ...
    fabric_pod = await client.get(
        kind=NetworkPod, branch=branch, parent__ids=[fabric.id], role__value=FABRIC_POD_ROLE
    )
    ...
    async def all_pods_have_spines() -> bool:
        for pod in pods:
            if await count_devices_in_pod(client, branch=branch, pod_id=pod.id, role="spine") < pod.amount_of_spines.value:
                return False
        return True
