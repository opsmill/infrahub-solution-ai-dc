...
async def generate_tenant(client: InfrahubClient) -> None:
    ...
    await run_generator(client, definition_name="generate-tenant", branch=OVERLAY_BRANCH, node_ids=[tenant.id])
...
async def leaves_with_segments(client: InfrahubClient) -> list[NetworkDevice]:
    ...
    leaves = await client.filters(kind=NetworkDevice, branch=OVERLAY_BRANCH, role__value="leaf", include=["segments"])
