...
async def generate_services(client: InfrahubClient, *, service_names: tuple[str, ...] | list[str]) -> None:
    ...
    async def all_servers_converged() -> bool:
        pending.clear()
        servers = {
            str(server.hostname.value): server
            for server in await client.filters(kind=NetworkServer, branch=SERVER_BRANCH)
        }
    ...
    await wait_until(
        ...
        what=lambda: (
            "generate-server to finish: " + ", ".join(f"{host} ({reason})" for host, reason in sorted(pending.items()))
        ),
...
class TestServerServiceL3(TestInfrahubDockerClient):
    ...
    async def test_server_is_not_an_artifact_target(self, client: InfrahubClient) -> None:
        ...
        live 1.11.0b2 stack, no ``Startup configuration`` artifact exists for *any* device: the
    ...
    async def test_explicit_occupied_port_fails_loud(self, client: InfrahubClient) -> None:
        ...
        await run_generator(client, definition_name="generate-server", branch=SERVER_BRANCH, node_ids=[service.id])
